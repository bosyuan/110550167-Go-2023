...
func InitObservable() {
	swearWord := loadFile("swear_word.txt")
	sensitiveWord := loadFile("sensitive_name.txt")

	ObservableMsg = ObservableMsg.Filter(func(item interface{}) bool {
		msg := item.(string)
		for _, word := range swearWord {
		...
		for _, word := range sensitiveWord {
			if strings.Contains(msg, word) {
				replaceWord := replaceAtIndex(word, '*', 1)
				msg = strings.ReplaceAll(msg, word, replaceWord)
			}
		}
		return msg, nil
	})
